...
#  expects tag_options[:id] to override object-method naming scheme in original
  def text_field_with_auto_complete2(object, method, tag_options = {}, completion_options = {})
    text_field_id = tag_options[:id]
    (completion_options[:skip_style] ? "" : auto_complete_stylesheet) +
    text_field(object, method, tag_options) +
    content_tag("div", "", :id => "#{text_field_id}_auto_complete", :class => "auto_complete") +
    auto_complete_field("#{text_field_id}", { :url => { :action => "auto_complete_for_#{object}_#{method}" } }.update(completion_options))
  end
